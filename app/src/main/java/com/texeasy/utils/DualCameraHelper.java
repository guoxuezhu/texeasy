    ...
    public List<Camera.Size> getCommonSupportedPreviewSize() {
        ...
        try {
            if (rgbSupportedPreviewSizes == null) {
                ...
                rgbCamera.release();
                Logcat.d(KLog.TAG, "后置相机打开成功");
            }
        } catch (RuntimeException e) {
            Logcat.e(KLog.TAG, "后置相机打开失败");
            e.printStackTrace();
            rgbSupportedPreviewSizes = new ArrayList<>();
        }
        try {
            if (irSupportedPreviewSizes == null) {
                ...
                irCamera.release();
                Logcat.d(KLog.TAG, "前置相机打开成功");
            }
        } catch (RuntimeException e) {
            Logcat.e(KLog.TAG, "前置相机打开失败");
            e.printStackTrace();
...
//            List<Camera.Size> supportedPreviewSize = getCommonSupportedPreviewSize();
            ...
            List<Camera.Size> supportedPreviewSize = new ArrayList<>();
            List<Camera.Size> tempSupportedPreviewSize = mCamera.getParameters().getSupportedPreviewSizes();
            for (Camera.Size rgbPreviewSize : tempSupportedPreviewSize) {
                if (rgbPreviewSize.width < MIN_PREVIEW_WIDTH || rgbPreviewSize.height < MIN_PREVIEW_HEIGHT) {
                    continue;
                }
                supportedPreviewSize.add(rgbPreviewSize);
            }
            Logcat.d(KLog.TAG, "方向为rotation：" + rotation);
            displayOrientation = getCameraOri(rotation);
            displayOrientation = 270;
            if (ConfigCenter.newInstance().camera90.get() != null) {
                displayOrientation = ConfigCenter.newInstance().camera90.get() ? 90 : 270;
            }
            Logcat.d(KLog.TAG, "方向为displayOrientation：" + displayOrientation);
            try {
                ...
                Logcat.d(KLog.TAG, "start: " + previewSize.width + "x" + previewSize.height);
    ...
    private int getCameraOri(int rotation) {
        ...
        Logcat.d(KLog.TAG, "additionalRotation：" + additionalRotation);
        Logcat.d(KLog.TAG, "info.facing：" + info.facing);
        Logcat.d(KLog.TAG, "info.orientation：" + info.orientation);
        Logcat.d(KLog.TAG, "getCameraOri result：" + result);
        return result;
    }
...
//            start();
        ...
        public void onSurfaceTextureSizeChanged(SurfaceTexture surfaceTexture, int width, int height) {
            Logcat.d(KLog.TAG, "onSurfaceTextureSizeChanged: " + width + "  " + height);
        }
...
//            start();
    ...
    public static final class Builder {
        ...
        public DualCameraHelper build() {
            if (previewViewSize == null) {
                Logcat.e(KLog.TAG, "previewViewSize is null, now use default previewSize");
            }
            if (cameraListener == null) {
                Logcat.e(KLog.TAG, "cameraListener is null, callback will not be called");
            }
