...
public abstract class ReaderBase {
        ...
        public void run() {
            ...
            while (mShouldRunning) {
                try {
                    ...
                    onLostConnect();
                    Logcat.e(KLog.TAG+"---onLostConnect---", e.getMessage());
                    return;
                } catch (Exception e) {
                    onLostConnect();
                    Logcat.e(KLog.TAG+"---onLostConnect---", e.getMessage());
    ...
    private void runReceiveDataCallback(byte[] btAryReceiveData) {
        try {
//            Log.d("---runReceiveData---", StringTool.byteArrayToString(btAryReceiveData, 0, btAryReceiveData.length));
                ...
                if (btAryBuffer.length > nLoop + 1) {
                    if (btAryBuffer[nLoop] == HEAD.HEAD) {
                        ...
                        if (nLoop + 1 + nLen < btAryBuffer.length) {
                            ...
                            // nLen + 2);
                            Logcat.d(KLog.TAG+"---reciveData---", StringTool.byteArrayToString(btAryAnaly, 0, btAryAnaly.length));
    ...
    private int sendMessage(final byte[] btArySenderData) {
        String msg = StringTool.byteArrayToString(btArySenderData, 0, btArySenderData.length);
        Logcat.d(KLog.TAG+"---sendMessage---", msg);
        mExecutorService.submit(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    onLostConnect();
                    Logcat.e(KLog.TAG+"---sendMessage---", "onLostConnect IOException");
                } catch (Exception e) {
                    onLostConnect();
                    Logcat.e(KLog.TAG+"---sendMessage---", "onLostConnect" + e.getMessage());
                }
            }
        });
...
//		}//ly
    ...
    public final int setWorkMode(byte btReadId, boolean blnFast) {//ly
        byte btCmd = CMD.SET_WORK_MODE;
        byte[] btAryData = new byte[1];

        btAryData[0] = (byte) (blnFast ? 0x01 : 0x00);
    ...
    public final int getWorkMode(byte btReadId) {
        byte btCmd = CMD.GET_WORK_MODE;
    ...
    public final int getAntPhysicalConnectionStatus(byte btReadId) {
        byte btCmd = CMD.GET_ANT_PHYSICAL_CONNECTION_STATUS;
        byte[] btAryData = new byte[1];
